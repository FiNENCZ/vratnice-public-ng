...
import { RezimDetailuEnum } from '../../shared/enums/rezim-detailu.enum';
import { errorTxtFunction } from '../../shared/functions/error-txt.function';
...
import { ZavodControllerService, RidicControllerService, VozidloTypControllerService, StatControllerService } from '../../../../build/openapi';
import moment from 'moment';

@Component({
  selector: 'app-povoleni-vjezdu-vozidla',
  standalone: true,
  imports: [DialogModule, MessageModule, MultiSelectModule, CheckboxModule],
...
export class PovoleniVjezduVozidlaComponent {
  ...
  zrusitEditaceDetail() {
    this.formDetail?.form.markAsPristine();
    this.detail!.idPovoleniVjezduVozidla = this.idZaznamu;
    this.loadDetail(this.detail!, true);
    this.initializeRidicValues();
  }

  showDetailDialog(ridicDto: PovoleniVjezduVozidlaDto) {
    this.loadDetail(ridicDto, true);
    this.initializeRidicValues();
  }

  private loadDetail(povoleniVjezduVozidla: PovoleniVjezduVozidlaDto, nacitatDetail: boolean) {
    if (nacitatDetail) {
      this.detail = undefined;
      this.uiService.showSpinner();
      this.povoleniVjezduVozidlaControllerService.getDetailPovoleniVjezduVozidla(povoleniVjezduVozidla.idPovoleniVjezduVozidla!, this.translateService.currentLang).subscribe({
        next: (vysledek: PovoleniVjezduVozidlaDto) => {
          this.uiService.stopSpinner();
          this.detail = vysledek;
          this.idZaznamu = this.detail?.idPovoleniVjezduVozidla;
          this.initializeRidicValues();
          this.nastavDatumy();
        },
        error: error => {
      ...
      this.detail = povoleniVjezduVozidla;
      this.idZaznamu = this.detail?.idPovoleniVjezduVozidla;
      this.initializeRidicValues();
      this.nastavDatumy();
    }
